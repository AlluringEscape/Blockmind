...
def look_around(x=0, y=0):
    try:
        # Capture the screen at specified coordinates (or full screen by default)
        img = pyautogui.screenshot(region=(x, y, 800, 600))
        
        # Analyze the frame using BlockMind Vision
        entities, items = blockmind_vision.analyze_frame(img)
        
        if len(entities) > 0 or len(items) > 0:
            logging.info("Detected entities/items: " + str(entities) + " and " + str(items))
            return True
        else:
        ...
        logging.error(f"An error occurred while scanning the area: {e}")
        return False
def gather_info():
