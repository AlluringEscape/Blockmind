...
def gather_info(item_type=None, entity_type=None):
    """
    Perform the Minecraft task: gather info.

    Args:
        item_type (str, optional): The type of item to look for. Defaults to None.
        entity_type (str, optional): The type of entity to look for. Defaults to None.
    ...
    try:
        ...
        # Analyze the frame for items and entities
        result = analyze_frame(screenshot)

        if item_type is not None and entity_type is not None:
            # If both item and entity types are specified, look for them in the result
            logger.info(f'Scanning for {item_type} items and {entity_type} entities...')
            meaningful_detected = (item_type in result['items'] or entity_type in result['entities'])
        elif item_type is not None:
            # If only an item type is specified, look for it in the result
            logger.info(f'Scanning for {item_type} items...')
            meaningful_detected = item_type in result.get('items', [])
        elif entity_type is not None:
            # If only an entity type is specified, look for it in the result
            logger.info(f'Scanning for {entity_type} entities...')
            meaningful_detected = entity_type in result.get('entities', [])

        return meaningful_detected

    except Exception as e:
        logger.error(f'Error gathering info: {str(e)}')
        return False
...
def look_around(context=None):
    """
    Perform a 360-degree scan around the bot to detect meaningful items or entities.
    
    Args:
        context (dict): Optional context-aware arguments for vision analysis.
    ...
    try:
        ...
        entities = analyze_frame(screenshot, context=context)
        
        # If any entities are found, log and return True
        if entities:
            logging.info(f'Detected {len(entities)} entity/entities in the 360-degree scan.')
            return True
        
    except Exception as e:
        # Log any exception that occurred during analysis
        logging.error(f'Error analyzing frame: {str(e)}')
    
    # If no entities are found, log and return False
    logging.info('No meaningful items or entities detected in the 360-degree scan.')
    return False
